<template>
    ...
    <div class="d-flex" id="wrapper">
        ...
        <div id="page-content-wrapper">
            ...
            <div class="container-fluid px-4">
              <h6 class="fs-2 m-0">Mã đơn hàng: {{ chitietdonhang.idDonHang }}</h6>
              <br>
      ...
      <tbody>
          <tr v-for="(chitietdonhang,index) in chitietdonhangs" v-bind:key="chitietdonhang.id">
            <td>{{index +1 }}</td>
              <td>{{chitietdonhang.tenSanPham}}</td>
              <td>{{ formatCurrency(chitietdonhang.giaBan * chitietdonhang.soLuong)}}</td>
  ...
  export default {
    ...
    created(){
      ...
      axios.get("https://localhost:7096/api/ChiTietDonHang/donhang/"+this.$route.params.id)
      .then(response=>{
        console.log(response.data);
          if(response.data){
              this.chitietdonhangs = response.data
          }
      })
      .catch(e=>{
          console.log("lỗi",e)
      })
      this.chitietdonhang.idDonHang =  this.$route.params.id
    if(this.chitietdonhang.idDonHang) {
      axios.get(`https://localhost:7096/api/DonHang/${this.chitietdonhang.idDonHang}`).then(response => {
