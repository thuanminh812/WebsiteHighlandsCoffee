<template>
    ...
    <br>
      ...
      <input placeholder="Tìm kiếm cụ thể" type="text" class="form-control ml-2 mr-2" name="tim" v-model="search" >
          ...
          <h4 style="font-weight: bold;">Khu vực: {{ khuvuc.tenKhuVuc }}</h4>
        <div class="row justify-content-center">
      <v-card elevation="10" v-for="cuahang in fillterCuaHang" v-bind:key="cuahang.id" class="col-sm-4 m-3">
            <v-card-title>
  ...
  export default {
   ...
   mounted() {
    axios.get("https://localhost:7096/api/KhuVuc/"+this.$route.params.id)
    .then((response) => this.setCuaHang(response.data))
