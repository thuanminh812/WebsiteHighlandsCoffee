<template>
    <div>
      <div class="d-flex" id="wrapper">
          ...
          <div id="page-content-wrapper">
              ...
              <button class="btn btn-info mb-3" style="margin-left:16px"><router-link to="formdmuctintuc" class="text-white">Thêm mới</router-link></button>
        ...
        <tbody>
            <tr v-for="danhmuctintuc in danhmuctintucs" v-bind:key="danhmuctintuc.tinTucId">
                <td>{{danhmuctintuc.tenDanhMuc}}</td>
                <td>{{danhmuctintuc.motaDanhMuc}}</td>
				<router-link :to="{ name: 'editdmuctintuc', params: {id: danhmuctintuc.tinTucId}}">
				<button class="btn btn-success"><i class="fa-solid fa-pen-to-square"></i></button> |
			</router-link>
				<button class="btn btn-danger" @click="onDelete(danhmuctintuc.tinTucId)"><i class="fa-solid fa-trash"></i></button> |
        <router-link :to="{ name: 'listtintuc', params: {id: danhmuctintuc.tinTucId}}">
  ...
  if (result.isConfirmed) {
	axios.delete(`https://localhost:7096/api/DanhMucTinTuc/${id}`,this.getTokenConfig()) .then(response => {
