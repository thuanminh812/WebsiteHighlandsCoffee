      ...
      <div class="d-flex" id="wrapper">
          ...
          <div class="form-group">
            <label>Tên danh mục</label>
              <input type="ten" class="form-control" name="ten" v-model="danhmuctintuc.tenDanhMuc" required>
          </div>
          <div class="form-group">
            <label>Mô tả danh mục</label>
              <textarea style="resize: none; width: 100%;
  height: 150px;box-sizing: border-box;" type="sdt" class="form-control" name="sdt" v-model="danhmuctintuc.motaDanhMuc" required></textarea>
          </div>
          <div>
    ...
    export default {
      ...
      methods:{
        decodeJwt(jwttoken){
          ...
          getDMucTinTuc(id) {
          axios.get(`https://localhost:7096/api/DanhMucTinTuc/${id}`).then(response => {
              this.danhmuctintuc = response.data
          })
          
      },
      save(){ 
        if(this.danhmuctintuc.tinTucId) {
          axios.put(`https://localhost:7096/api/DanhMucTinTuc/${this.danhmuctintuc.tinTucId}`,this.danhmuctintuc,this.getTokenConfig()).then(response => {
              if(response.data){
                this.$swal.fire('Cập nhật thành công!','','success')
                  this.$router.push("/listdmuctintuc")
                  return
              }
          })
          return;
