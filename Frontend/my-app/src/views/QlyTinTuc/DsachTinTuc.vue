<template>
          ...
          <div id="page-content-wrapper">
              <nav class="navbar navbar-expand-lg navbar-light bg-transparent py-4 px-4">
                  <div class="d-flex align-items-center">
                      ...
                      <h2 class="fs-2 m-0">Danh sách tin tức</h2>
                  </div>
  ...
  height: 150px;box-sizing: border-box;"  class="form-control"  v-model="tintuc.motaTinTuc" required></textarea>
        ...
        <tbody>
            <tr  v-for="tintuc in tintucs" v-bind:key="tintuc.id">
                <v-img style="width:160px;height:160px" class="card-image" :src="`${tintuc.hinhAnh}`"></v-img>
                <td style="width: 180px">{{tintuc.tieuDe}}</td>
                <td style="width: 400px">{{tintuc.linkUrl}}</td>
                <td style="width: 600px">{{tintuc.motaTinTuc}}</td>
				<router-link :to="{ name: 'edittintuc', params: {id: tintuc.id}}">
				<button class="btn btn-success"><i class="fa-solid fa-pen-to-square"></i></button> |
			</router-link>
				<button class="btn btn-danger" @click="onDelete(tintuc.id)"><i class="fa-solid fa-trash"></i></button>
            </tr>
        </tbody>
    </table>
              </div>
    ...
    export default {
      ...
      created(){
        ...
        axios.get("https://localhost:7096/api/TinTuc/tintucid/"+this.$route.params.id)
        .then(response=>{
            if(response.data){
                this.tintucs = response.data
            }
        })
        this.tintuc.tinTucId =  this.$route.params.id
      if(this.tintuc.tinTucId) {
        axios.get(`https://localhost:7096/api/DanhMucTinTuc/${this.tintuc.tinTucId}`).then(response => {
  ...
  if (result.isConfirmed) {
	axios.delete(`https://localhost:7096/api/TinTuc/${id}`,this.getTokenConfig()) .then(response => {
        ...
        this.tintuc.hinhAnh = this.imgURL,
          axios.post('https://localhost:7096/api/TinTuc',this.tintuc,this.getTokenConfig()).then(response => {
              if(response.data){
