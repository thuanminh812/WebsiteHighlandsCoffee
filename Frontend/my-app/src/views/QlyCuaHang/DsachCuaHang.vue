<template>
        ...
        <div class="modal-wrapper">
          ...
          <div class="form-group">
            <label>Tên cửa hàng</label>
              <input class="form-control"  v-model="cuahang.tenCuaHang" required>
          </div>
          <div class="form-group">
  ...
  height: 150px;box-sizing: border-box;"  class="form-control"  v-model="cuahang.diaChi" required></textarea>
          </div>
          <div class="form-group">
            <label>Link bản đồ</label>
              <input class="form-control"  v-model="cuahang.linkBanDo" required>
          </div>
          <div>
        ...
        <tbody>
            <tr v-for="cuahang in cuahangs" v-bind:key="cuahang.id">
              <v-img style="width:160px;height:160px" class="card-image" :src="`${cuahang.hinhAnh}`"></v-img>
                <td>{{cuahang.tenCuaHang}}</td>
                <td>{{cuahang.diaChi}}</td>
                <td>{{cuahang.linkBanDo}}</td>
				<router-link :to="{ name: 'editcuahang', params: {id: cuahang.id}}">
				<button class="btn btn-success"><i class="fa-solid fa-pen-to-square"></i></button> |
			</router-link>
				<button class="btn btn-danger" @click="onDelete(cuahang.id)"><i class="fa-solid fa-trash"></i></button> 
        <router-link :to="{ name: 'listcuahang', params: {id: cuahang.id}}">
			</router-link>
            </tr>
        </tbody>
    </table>
    ...
    export default {
      ...
      created(){
        ...
        axios.get("https://localhost:7096/api/CuaHang/cuahangid/"+this.$route.params.id)
        .then(response=>{
        ...
        this.cuahang.idKhuVuc =  this.$route.params.id
      if(this.cuahang.idKhuVuc) {
        axios.get(`https://localhost:7096/api/KhuVuc/${this.cuahang.idKhuVuc}`).then(response => {
  ...
  if (result.isConfirmed) {
	axios.delete(`https://localhost:7096/api/CuaHang/${id}`,this.getTokenConfig()) .then(response => {
		console.log(response.data)
		if(response.data){
    ...
    save(){ 
        this.cuahang.hinhAnh = this.imgURL,
          axios.post('https://localhost:7096/api/CuaHang',this.cuahang,this.getTokenConfig()).then(response => {
              if(response.data){
