    ...
    export default {
      ...
      methods:{
        decodeJwt(jwttoken){
          ...
          getCuaHang(id) {
          axios.get(`https://localhost:7096/api/CuaHang/${id}`).then(response => {
              this.cuahang = response.data
          })
      ...
      save(){ 
        ...
        if(this.cuahang.id) {
          axios.put(`https://localhost:7096/api/CuaHang/${this.cuahang.id}`,this.cuahang,this.getTokenConfig()).then(response => {
              if(response.data){
