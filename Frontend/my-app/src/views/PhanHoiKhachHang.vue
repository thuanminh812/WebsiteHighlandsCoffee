<template>
    <div>
      <div class="d-flex" id="wrapper">
          ...
          <div id="page-content-wrapper">
              <nav class="navbar navbar-expand-lg navbar-light bg-transparent py-4 px-4">
                  <div class="d-flex align-items-center">
                      ...
                      <h2 class="fs-2 m-0">Phản hồi khách hàng</h2>
                  </div>
        ...
        <tbody>
            <tr  v-for="phanhoi in phanhois" v-bind:key="phanhoi.id">
                <td style="width: 180px">{{phanhoi.email}}</td>
                <td style="width: 400px">{{phanhoi.hoTen}}</td>
                <td style="width: 600px">{{phanhoi.soDienThoai}}</td>
                <td style="width: 600px">{{phanhoi.noiDung}}</td>
				<button class="btn btn-danger" @click="onDelete(phanhoi.id)"><i class="fa-solid fa-trash"></i></button>
            </tr>
        </tbody>
    </table>
              </div>
    ...
    export default {
      ...
      created(){
        ...
        axios.get("https://localhost:7096/api/PhanHoi")
        .then(response=>{
            if(response.data){
                this.phanhois = response.data
            }
        })
      },
      methods:{
  ...
  if (result.isConfirmed) {
	axios.delete(`https://localhost:7096/api/PhanHoi/${id}`,this.getTokenConfig()) .then(response => {
