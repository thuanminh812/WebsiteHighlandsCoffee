      ...
      <div class="d-flex" id="wrapper">
          ...
          <div class="form-group">
            <label>Tên danh mục</label>
              <input type="ten" class="form-control" name="ten" v-model="danhmucmenu.tenDanhMuc" required>
          </div>
          <div class="form-group">
            <label>Mô tả danh mục</label>
              <textarea style="resize: none; width: 100%;
  height: 150px;box-sizing: border-box;" type="sdt" class="form-control" name="sdt" v-model="danhmucmenu.motaDanhMuc" required></textarea>
          </div>
          <div>
    ...
    export default {
      ...
      methods:{
        decodeJwt(jwttoken){
          ...
          getDMucMenu(id) {
          axios.get(`https://localhost:7096/api/DanhMucMenu/${id}`).then(response => {
              this.danhmucmenu = response.data
          })
          
      },
      save(){ 
        if(this.danhmucmenu.menuId) {
          axios.put(`https://localhost:7096/api/DanhMucMenu/${this.danhmucmenu.menuId}`,this.danhmucmenu,this.getTokenConfig()).then(response => {
              if(response.data){
                this.$swal.fire('Cập nhật thành công!','','success')
                  this.$router.push("/listdmucmenu")
                  return
              }
          })
          return;
