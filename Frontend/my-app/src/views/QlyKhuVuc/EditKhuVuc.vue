      ...
      <div class="d-flex" id="wrapper">
          ...
          <div class="form-group">
            <label>Tên khu vực</label>
              <input type="ten" class="form-control" name="ten" v-model="khuvuc.tenKhuVuc" required>
          </div>
          <div>
    ...
    export default {
      ...
      methods:{
        decodeJwt(jwttoken){
          ...
          getDMucTinTuc(id) {
          axios.get(`https://localhost:7096/api/KhuVuc/${id}`).then(response => {
              this.khuvuc = response.data
          })
      ...
      save(){ 
        if(this.khuvuc.idKhuVuc) {
          axios.put(`https://localhost:7096/api/KhuVuc/${this.khuvuc.idKhuVuc}`,this.khuvuc,this.getTokenConfig()).then(response => {
              if(response.data){
                this.$swal.fire('Cập nhật thành công!','','success')
                  this.$router.push("/qlykhuvuc")
                  return
              }
          })
          return;
